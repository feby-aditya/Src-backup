 ...
 *
 * This Artificial Neural Network architecture can be used to perform
 * classification, regression or time-series prediction.
 */
MLP::MLP(byte inputs, byte outputs, int *topology, byte activFunc,
         double const *weights, boolean usingPGM) {
  _inflection = 1.0;
...
double *MLP::getI(int inputs, int layer, double *sample) {
  int i, j, ISize = _layerSizes[layer],
            wLen = (layer < 1 ? inputs : _layerSizes[layer - 1]) + 1;
  double *I = _iArray;
  _iOffset = ISize;
...
double *MLP::getY(int inputs, int layer, double *sample) {
  int i, last = _layerSizes[layer + 1] == -1,
         YSize = _layerSizes[layer] + !last;
